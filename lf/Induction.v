...
(** **** Exercise: 2 stars, advanced, especially useful (add_comm_informal)
    ...
    - First, suppose [n = 0].  We must show that
      ...
      This follows from the definition of [+] for the left hand side,
      and from the theorem add_O_r for the right hand side.
    ...
    - Next, suppose [n = S n'], where
      ...
      By the definition of [+], this becomes

        S (n' + m) = m + S n',

      Which we can rewrite as, using induction hypothesis :

       S (m + n') = m + S n',

      Which is true because of the add_n_Sm theorem. __Qed.__
...
Proof.
  ...
  - simpl. rewrite IHn'. assert (H : forall l p : nat, l + l * p = l * S p).
   {intros l p. induction l as [|l' IHl'].
    - reflexivity. 
    - simpl. rewrite <- IHl'. rewrite add_assoc. rewrite add_assoc. rewrite add_comm. rewrite <- add_comm.
    assert (H' : l' + p = p + l'). {rewrite add_comm. reflexivity. } rewrite H'. reflexivity. }
...
Theorem andb_false_r : forall b : bool,
  andb b false = false.
Proof.
  intros b. destruct b as [] eqn : Eqb.
  - reflexivity.
  - reflexivity.
Qed.
...
Theorem all3_spec : forall b c : bool,
  ...
  = true.
Proof.
  intros b c. destruct b as [] eqn : Eqb.
  - simpl. destruct c as [] eqn : Eqc. 
    + reflexivity.
    + reflexivity.
  - reflexivity.
Qed. 
...
Proof.
  ...
  - reflexivity.
  - reflexivity.
  - simpl. rewrite IHb'. simpl. replace (bin_to_nat b'' + S(bin_to_nat b'')) with (S(bin_to_nat b'' + bin_to_nat b'')).
   + reflexivity.
...
Proof.
  intros n. destruct n as [| n'] eqn : Eqn'.
  - reflexivity.
  - reflexivity.
Qed.
...
(* FILL IN HERE *)
Example normalize_all_zeros : normalize(B0 (B0 (Z))) = Z.
Proof.
  reflexivity.
Qed.
Example normalize_with_one : normalize(B1(B0(B1(B0(B0 Z))))) = (B1 (B0 (B1 Z))).
Proof.
  reflexivity.
Qed.
...
Proof.
  assert (H : forall n : nat, nat_to_bin(double n) = double_bin(nat_to_bin n)).
      { intros n. induction n as [| n' IHn'].
        * reflexivity.
        * simpl. rewrite double_incr_bin. rewrite IHn'. reflexivity. }
  intros b. induction b as [|b' IHb'| b'' IHb''].
  - reflexivity.
  - simpl. replace (normalize b') with (nat_to_bin(bin_to_nat b')). 
    rewrite H. reflexivity.
  - simpl. replace (normalize b'') with (nat_to_bin(bin_to_nat b'')). 
    rewrite H. 
    assert (H' : forall b : bin, incr(double_bin b) = B1 b). 
      { intros b. destruct b as [| b0 | b1 ] eqn:Eqb. 
        - reflexivity.
        - reflexivity.
        - reflexivity. }
    rewrite H'. reflexivity.
