...
Proof.
  intros [|n'] m H.
  + left. reflexivity.
  + right. simpl in H. destruct m as [|m'].
    - reflexivity.
...
Theorem not_implies_our_not : forall (P:Prop),
  ~ P -> (forall (Q:Prop), P -> Q).
Proof.
  intros P notP. intros Q H. unfold not in notP. apply notP in H. destruct H.
Qed.
(** [] *)
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros P Q H. intros notQ. unfold not in notQ. unfold not. intros H'. apply H in H'. 
  apply notQ in H'. destruct H'.
  Qed.
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof.
  intros P. unfold not. intros [HP HnP]. apply HnP in HP. destruct HP.
Qed.
  (** [] *)
...
Theorem iff_trans : forall P Q R : Prop,
  (P <-> Q) -> (Q <-> R) -> (P <-> R).
Proof.
  intros P Q R Hiff1 Hiff2. split.
...
Proof.
  intros P Q R. split.
  + intros H. destruct H as [Hleft | Hright]. 
    - split. left. apply Hleft. left. apply Hleft.
    - destruct Hright as [HQ HR]. split. right. apply HQ. right. apply HR.
  + intros [Hl Hr]. destruct Hl as [HP | HQ].
    - left. apply HP.
...
Proof.
  intros X P. intros H. unfold not. intros [x H']. apply H'. apply H.
Qed.
  (** [] *)
...
Proof.
  intros X P Q. split. 
  - intros [x Hx]. destruct Hx as [H1 | H2].
    + left. exists x. apply H1.
    + right. exists x. apply H2.
  - intros H. destruct H as [Hl | Hr]. 
    + destruct Hl as [x Hx]. exists x. left. apply Hx.
    + destruct Hr as [x Hx]. exists x. right. apply Hx.
Qed.
(** [] *)
...
Proof.
  ...
  - intros n H. simpl in H. assert (H1 : forall n, (n<=? 0) = true -> n = 0). {
    intros [|n'] H2. reflexivity. simpl in H2. discriminate H2.
  } apply H1 in H. exists 0. rewrite <- plus_n_O. symmetry. apply H. 
  - intros n H. destruct n as [| n']. 
    + exists (S m'). reflexivity.
    + simpl in H. apply IHm' in H. destruct H as [x Hx]. exists x. simpl. rewrite Hx. reflexivity.
Qed.

Theorem plus_exists_leb : forall n m, (exists x, m = n+x) -> n <=? m = true.
Proof.
  intros n m [x Hx]. rewrite Hx. assert (H : forall n x, (n <=? n+x = true)). 
  {
  ...
  - induction l as [|h t IHt].
    ...
    + simpl. intros [H | H].
      * exists h. split.
        -- apply H.
        -- left. reflexivity.
      * apply IHt in H. destruct H as [x Hx]. exists x.
      split.
        -- destruct Hx as [H1 H2]. apply H1.
        -- destruct Hx as [H1 H2]. right. apply H2.
  - induction l as [|h t IHt].
    + simpl. intros [_ [_ []]].
    + intros [x [H1 H2]]. simpl in H2. destruct H2 as [H2 | H2].
      * rewrite <- H2 in H1. simpl. left. apply H1.
      * simpl. right. apply IHt. exists x. split. apply H1. apply H2. 
Qed.
(** [] *)
...
Proof.
  intros A l. induction l as [|a' l' IH].
    ...
    - simpl. split.
      + intros [H |H ].
        * left. left. apply H.
        * apply IH in H. rewrite <- or_assoc. right. apply H.
      + intros [H1 | H2].
...
Proof.
  ...
  - simpl. split.
    ...
    + intros [H1 H2]. intros x H.
    destruct H as [H | H].
      * rewrite <- H. apply H1.
      * rewrite <- IH in H2. apply H2. apply H.
Qed.
  (** [] *)
