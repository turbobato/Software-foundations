...
Theorem app_assoc : forall A (l m n:list A),
  l ++ m ++ n = (l ++ m) ++ n.
Proof.
  intros A. intros l m n. induction l as [|h t IHt].
  - reflexivity.
  - simpl. rewrite IHt. reflexivity.
Qed.
...
Proof.
  intros X. intros l1 l2. induction l1 as [|h t IHt].
  - reflexivity.
  - simpl. rewrite IHt. reflexivity.
Qed.
(** [] *)
...
(** **** Exercise: 1 star, standard, optional (combine_checks)
    ...
    - What is the type of [combine] (i.e., what does [Check
      @combine] print?) forall X Y: Type, list X -> list Y -> list (X*Y)
    - What does

        Compute (combine [1;2] [false;false;true;true]).

      print?
        It should print [(1,false);(2,false)]
    [] *)
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. assert (H: forall (X Y : Type) (f : X -> Y) (l1 l2 : list X), map f (l1 ++ l2) = map f l1 ++ map f l2).
    { intros X0 Y0 f0 l1 l2. induction l1 as [|h1 t1 IHt1]. 
      * reflexivity.
      * simpl. rewrite IHt1. reflexivity. }
    rewrite H. rewrite IHt. simpl. reflexivity.
Qed. 
(** [] *)
...
(* FILL IN HERE
  If X is nat and Y is natlist, then the operator could be cons for instance
    [] *)
